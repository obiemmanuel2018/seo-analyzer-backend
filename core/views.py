...
from payment.utils import add_months
...
class Signup(APIView):
    ...
    @swagger_auto_schema(
        responses={200: SignupSerializer(many=True)}, request_body=SignupSerializer
    )
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            subscription = Subscription.objects.create(
                id=user.email, user=user, valid_to=add_months(datetime.date.today(), 1)
            )
...
class ResendVerificationLink(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            send_verification_mail(request, user)
            return Response(
                {"message": "Verification Linked Sent Successfully"},
...
class Logout(APIView):
    ...
    @swagger_auto_schema(
        responses={200: LogoutSerializer(many=True)}, request_body=LogoutSerializer
    )
    def post(self, request, format=None):
        """
        Log User Out
...
class PasswordReset(APIView):
    ...
    @swagger_auto_schema(
        responses={200: PasswordResetSerializer(many=True)},
...
class PasswordResetVerified(APIView):
    ...
    @swagger_auto_schema(
        responses={200: PasswordResetVerifiedSerializer(many=True)},
...
class EmailChange(APIView):
    ...
    @swagger_auto_schema(
        responses={200: EmailChangeSerializer(many=True)},
...
class PasswordChange(APIView):
    ...
    @swagger_auto_schema(
        responses={200: PasswordChangeSerializer(many=True)},
